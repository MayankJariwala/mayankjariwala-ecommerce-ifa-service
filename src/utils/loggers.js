...
Loggers.prototype.log = function (layer = __filename, message) {
	console.log(`[${get_file_name(layer)}] [${date.toISOString()}] ${message}`);
};

Loggers.prototype.error = function (layer = __filename, message) {
	console.error(`[${get_file_name(layer)}] [${date.toISOString()}] ${message}`);
};

Loggers.prototype.exception = function (layer = __filename, message) {
	console.warn(`[${get_file_name(layer)}] [${date.toISOString()}] ${message}`);
};

Loggers.prototype.debug = function (layer = __filename, message) {
	console.debug(`[${get_file_name(layer)}] [${date.toISOString()}] ${message}`);
};

Loggers.prototype.warn = function (layer = __filename, message) {
	console.warn(`[${get_file_name(layer)}] [${date.toISOString()}] ${message}`);
};
